...
//UserDefine
#include"../Widget/LineEditorEnterWidget.h"
class LineEditorEnterFactoryPrivate:public EditorFactoryPrivate<LineEditorEnterWidget>
{
    LineEditorEnterFactory* q_ptr;
    Q_DECLARE_PUBLIC(LineEditorEnterFactory)
public:
    void slotPropertyChanged(QtProperty* property, const QString& value);
    void slotSetValue(const QString& value);
};

void LineEditorEnterFactoryPrivate::slotPropertyChanged(QtProperty* property,const QString& value)
{
    const auto it = m_createdEditors.constFind(property);
    if (it == m_createdEditors.constEnd())
        return;

    for (LineEditorEnterWidget* editor : it.value()) {
        editor->setValue(value);
    }
}

void LineEditorEnterFactoryPrivate::slotSetValue(const QString& value)
{
    QObject* object = q_ptr->sender();
    const QMap<LineEditorEnterWidget*, QtProperty*>::ConstIterator ecend = m_editorToProperty.constEnd();
    for (QMap<LineEditorEnterWidget*, QtProperty*>::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != ecend; ++itEditor)
        if (itEditor.key() == object) {
            QtProperty* property = itEditor.value();
            QtStringPropertyManager* manager = q_ptr->propertyManager(property);
            if (!manager)
...
//###############
LineEditorEnterFactory::LineEditorEnterFactory(QObject* parent)
    : QtAbstractEditorFactory<QtStringPropertyManager>(parent), d_ptr(new LineEditorEnterFactoryPrivate())
{
    d_ptr->q_ptr = this;

}

LineEditorEnterFactory::~LineEditorEnterFactory()
{
    qDeleteAll(d_ptr->m_editorToProperty.keys());
}

void LineEditorEnterFactory::connectPropertyManager(QtStringPropertyManager* manager)
{
    connect(manager, SIGNAL(valueChanged(QtProperty*, QString)),
        this, SLOT(slotPropertyChanged(QtProperty*, QString)));
}

QWidget* LineEditorEnterFactory::createEditor(QtStringPropertyManager* manager,
    QtProperty* property, QWidget* parent)
{

    LineEditorEnterWidget* editor = d_ptr->createEditor(property, parent);
    editor->setValue(manager->value(property));
...
void LineEditorEnterFactory::disconnectPropertyManager(QtStringPropertyManager* manager)
{
    disconnect(manager, SIGNAL(valueChanged(QtProperty*, QString)),
        this, SLOT(slotPropertyChanged(QtProperty*, QString)));
}

#include"../Widget/PixmapSelectedWidget.h"

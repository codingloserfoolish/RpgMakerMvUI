...
//UserDefine
class LineEditorEnterFactoryPrivate;
class LineEditorEnterFactory :public QtAbstractEditorFactory<QtStringPropertyManager>
{
    Q_OBJECT
public:
    LineEditorEnterFactory(QObject* parent = nullptr);
    ~LineEditorEnterFactory();
protected:
    virtual void connectPropertyManager(QtStringPropertyManager*manager)override;
    virtual QWidget*createEditor(QtStringPropertyManager*manager,QtProperty*property,QWidget* parent)override;
    virtual void disconnectPropertyManager(QtStringPropertyManager* manager)override;
private:
    QScopedPointer<LineEditorEnterFactoryPrivate> d_ptr;
    Q_DECLARE_PRIVATE(LineEditorEnterFactory)
    Q_DISABLE_COPY_MOVE(LineEditorEnterFactory)
    Q_PRIVATE_SLOT(d_func(), void slotPropertyChanged(QtProperty* property, const QString& value))

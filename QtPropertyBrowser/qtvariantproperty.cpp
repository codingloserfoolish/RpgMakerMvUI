...
* Value:1326
* SetValue:1587
* QtVariantEditorFactoryPrivate:1868
* QtVariantEditorFactory:1960 
* connectPropertyManager:2034
* disconnectPropertyManager:2156
*/
#define LINE_EXTRA 1
...
public:
    ...
    //UserDefine
    void slotValueChanged(QtProperty* property, const PixmapNode& val);
    //DefineEnd
...
void QtVariantPropertyManagerPrivate::slotValueChanged(QtProperty* property, const PixmapNode& val)
{
    ...
    : QtAbstractPropertyManager(parent), d_ptr(new QtVariantPropertyManagerPrivate)
{
    ...
    //UserDefine
    int pixmapId = qMetaTypeId<PixmapNode>();
    PixmapPropertyManager* pixmapPropertyManager = new PixmapPropertyManager(this);
    d_ptr->m_typeToPropertyManager[pixmapId] = pixmapPropertyManager;
    d_ptr->m_typeToValueType[pixmapId] = QVariant::String;
    connect(pixmapPropertyManager, SIGNAL(valueChanged(QtProperty*, PixmapNode)),
        this, SLOT(slotValueChanged(QtProperty*, PixmapNode)));
    //DefineEnd
}
...
QVariant QtVariantPropertyManager::value(const QtProperty *property) const
{
    ...
    }else if (PixmapPropertyManager* pixmapManager = qobject_cast<PixmapPropertyManager*>(manager)) {
        return QVariant::fromValue(pixmapManager->value(internProp));
    }
    return QVariant();
}
...
void QtVariantPropertyManager::setValue(QtProperty *property, const QVariant &val)
{
    ...
    if (QtIntPropertyManager *intManager = qobject_cast<QtIntPropertyManager *>(manager)) {
        ...
        pixmapManager->setValue(internProp, qvariant_cast<PixmapNode>(val));
        return;
    }
}
...
public:
    ...
    //UserDefine
    LineEditorEnterFactory     *m_lineEditorEnterFactory;
    PixmapEditorFactory* m_pixmapEditorFactory;
    //DefineEnd
    ...
    : QtAbstractEditorFactory<QtVariantPropertyManager>(parent), d_ptr(new QtVariantEditorFactoryPrivate())
{
...
#else
    d_ptr->m_lineEditorEnterFactory = new LineEditorEnterFactory(this);
    d_ptr->m_factoryToType[d_ptr->m_lineEditorEnterFactory] = QVariant::String;
    d_ptr->m_typeToFactory[QVariant::String] = d_ptr->m_lineEditorEnterFactory;
#endif
    ...
    //UserDefine
    int pixmapId = qMetaTypeId<PixmapNode>();
    d_ptr->m_pixmapEditorFactory = new PixmapEditorFactory(this);
    d_ptr->m_factoryToType[d_ptr->m_pixmapEditorFactory] = pixmapId;
    d_ptr->m_typeToFactory[pixmapId] = d_ptr->m_pixmapEditorFactory;
    //DefineEnd
}
...
void QtVariantEditorFactory::connectPropertyManager(QtVariantPropertyManager *manager)
{
...
#else
        d_ptr->m_lineEditorEnterFactory->addPropertyManager(manager);
#endif
    ...
    //UserDefine
    const auto pixmapPropertyManagers = manager->findChildren<PixmapPropertyManager*>();
    for (PixmapPropertyManager* manager : pixmapPropertyManagers)
        d_ptr->m_pixmapEditorFactory->addPropertyManager(manager);
    //DefineEnd
}
...
void QtVariantEditorFactory::disconnectPropertyManager(QtVariantPropertyManager *manager)
{
...
#else
        d_ptr->m_lineEditorEnterFactory->removePropertyManager(manager);
#endif // LINE_EXTRA
    ...
    const auto pixmapPropertyManagers = manager->findChildren<PixmapPropertyManager*>();
    for (PixmapPropertyManager* manager : pixmapPropertyManagers)
        d_ptr->m_pixmapEditorFactory->removePropertyManager(manager);
    //DefineEnd
}

...
int IconSelectedWidget::_send_Id = 0;
...
IconSelectedWidget::IconSelectedWidget(QWidget* parent):QDialog(parent)
{
	...
	connect(pixwidget, &IconPicture::selectedChanged, this, &IconSelectedWidget::slot_indexChanged);
	connect(button, &QPushButton::clicked, this, &IconSelectedWidget::slot_setExecMsg);

}

int IconSelectedWidget::GetIconId()
{
	IconSelectedWidget w;
...
void IconSelectedWidget::slot_setExecMsg()
{
	if (_send_Id != 0)
	{
		this->accept();
...
void IconSelectedWidget::slot_indexChanged(int icon_index)
{
